...
(function () {
  ...
  const I18N = {
    ...
    /* ---- Theme toggle ---- */
    "theme.toDark": { en: "Switch to dark mode", he: "מעבר למצב כהה" },
    "theme.toLight": { en: "Switch to light mode", he: "מעבר למצב בהיר" },

    /* ---- Chat widget ---- */
    "chat.fabAria": { en: "Open chat", he: "פתיחת צ'אט" },
    "chat.name": { en: "Cheap Home Assistant", he: "העוזר של צ'יפ הום" },
    "chat.status": { en: "Typically replies instantly", he: "בדרך כלל עונה מיד" },
    "chat.closeAria": { en: "Close chat", he: "סגירת צ'אט" },
    "chat.welcome1": { en: "Hi! 👋 I'm the Cheap Home assistant.", he: "היי! 👋 אני העוזר הדיגיטלי של צ'יפ הום." },
    "chat.welcome2": { en: "Can I help you find an affordable home today?", he: "אפשר לעזור לכם למצוא היום בית במחיר משתלם?" },
    "chat.placeholder": { en: "Type a message…", he: "הקלידו הודעה…" },
    "chat.sendAria": { en: "Send message", he: "שליחת הודעה" },
    "chat.reply": {
      en: "Thanks for the message! This is a demo assistant — leave your details on our Contact page and a real team member will follow up.",
      he: "תודה על ההודעה! זהו עוזר הדגמה — השאירו פרטים בעמוד צור הקשר וחבר צוות אמיתי יחזור אליכם."
    },
    "chat.cta": { en: "Go to Contact page", he: "מעבר לעמוד צור קשר" }
  };
  ...
  /* ---------------------------------------------------------
     3b. Theme state (light / dark)
  --------------------------------------------------------- */
  const THEME_KEY = "cheaphome_theme";
  ...
  function refreshBrandLogos(lang, theme) {
    document.querySelectorAll("img[data-logo-en-light]").forEach(function (img) {
      const attr = "data-logo-" + (lang === "he" ? "he" : "en") + "-" + (theme === "dark" ? "dark" : "light");
  ...
  function applyTheme(theme) {
    ...
    document.querySelectorAll(".theme-toggle").forEach(function (btn) {
      ...
      if (entry) btn.setAttribute("aria-label", entry[getLang()]);
    });

    refreshBrandLogos(getLang(), theme);
  }
  ...
  function applyLang(lang) {
    ...
    refreshBrandLogos(lang, getTheme());
    applyTheme(getTheme());
  ...
  /* ---------------------------------------------------------
     9. Chat widget (Home & About only — gated on markup
     presence). Auto-opens once after 30s on page unless the
     visitor already opened or dismissed it this session.
  --------------------------------------------------------- */
  const CHAT_SEEN_KEY = "cheaphome_chat_seen";
  let chatMessagesRendered = false;
  ...
  function chatAppendBotCTA(text, ctaLabel, href) {
    const list = document.getElementById("chat-messages");
    if (!list) return;
    const div = document.createElement("div");
  ...
  function chatRenderWelcome(lang) {
    if (chatMessagesRendered) return;
    const list = document.getElementById("chat-messages");
    if (!list) return;
    list.innerHTML = "";
    chatAppendMessage("bot", I18N["chat.welcome1"][lang]);
    chatAppendMessage("bot", I18N["chat.welcome2"][lang]);
    chatMessagesRendered = true;
  }

  const CHAT_SOUND_SRC = "assets/chat-notify.mp3";
  const chatAudio = new Audio(CHAT_SOUND_SRC);
  chatAudio.preload = "auto";
  chatAudio.volume = 0.5;

  // Browsers block audio that isn't tied to a user gesture. Playing (and
  // instantly pausing) on the visitor's first interaction "unlocks" this
  // element so the later un-gestured play() at the 30s mark is allowed.
  let chatAudioUnlocked = false;
  function chatUnlockAudio() {
    if (chatAudioUnlocked) return;
    chatAudioUnlocked = true;
    const p = chatAudio.play();
    if (p && typeof p.then === "function") {
      p.then(function () { chatAudio.pause(); chatAudio.currentTime = 0; }).catch(function () {});
    }
  }
  ["click", "keydown", "touchstart"].forEach(function (evt) {
    document.addEventListener(evt, chatUnlockAudio, { once: true, passive: true });
  });
  ...
  function chatToggle() {
    const widget = document.getElementById("chat-widget");
    if (!widget) return;
    if (widget.classList.contains("open")) chatClose(); else chatOpen();
  }
  ...
  function chatSend() {
    ...
    setTimeout(function () {
      ...
      if (t) t.remove();
      const lang = getLang();
      chatAppendBotCTA(I18N["chat.reply"][lang], I18N["chat.cta"][lang], "contact.html");
    }, 900);
  }
  ...
  function initChat() {
    ...
    if (fab) fab.addEventListener("click", chatToggle);
    ...
    try { alreadySeen = sessionStorage.getItem(CHAT_SEEN_KEY) === "1"; } catch (e) {}
  ...
  /* ---------------------------------------------------------
     10. Contact form (demo only, no network calls)
  --------------------------------------------------------- */
  function initForm() {
